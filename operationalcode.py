        ...
        for name in glob.fnmatch.filter(f, '*osgb_vmsk_sharp_rad_srefdem_stdsref.tif'):
            ...
            if name.split('_')[1][4:6] not in months_out:
                ...
                if name.split('_')[3] in grantoproc:
                    ...
                    if getcloudmetadata(os.path.join(r, name)) < config.CLOUD:
                        # create [imagename, imagepath, granule, size, date]
                        paramlist = [name, r, name.split('_')[3], size, name.split('_')[1]] 
...
if __name__ == "__main__":
    ...
    cleanlist = []
    # Look for full scenes if J[3] > 1: remove to process all images (what is effect of null data?)
    for j in toprocess:
        if j[3] > 0:
            cleanlist.append(j)
