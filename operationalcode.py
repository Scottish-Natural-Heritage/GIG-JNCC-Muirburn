...
Description:
This code takes in a working and output directory which is validated, and searches for  images . The working directory is generated by a crawl of the working directory filesystem. 
The list of images is sorted by date. A cleaned, sorted list of files is fed into functions that read the imagery  as pre-burn and post-burn datasets and passes that to functions that calculate NBR, NBR2 and SAVI. These outputs are then thresholded to create a seed layer, ready for region growing. 
Finally the vector output and logfiles are exported.
Contributors: 
...
def getdatalist(wd, grantoproc, months_out, cloud_thresh1, cloud_thresh2):
    '''
...
def csi(swir2, nir):
    ...
    nir -- near infra-red band  
    '''
    csi = np.zeros(swir2.shape).astype(rasterio.float32)
    mask = swir2*nir!=0 # Deal with divide by zero
    csi[mask] = (-1 * (nir[mask]/swir2[mask]))

    return csi

def threshold_imgs(dsavi, postnbr, dnbr2, dcsi, thresholds):
    '''
    ...
    dcsi -- pre/post difference in CSI
    ...
    reclassArray[np.where(dcsi<=(thresholds['threshdcsi']))] = 0
...
def grow_burn(dsavi, postnbr, dnbr2, dcsi, thresholds):
    '''
    ...
    extendArray[np.where(dcsi<=(thresholds['shadowthresh']))] = 0
...
def gran_process (toprocess):
    ...
    for gran in config.PROC_GRANULES:
        ...
        while x < (len(toprocessx)-1):
            ...
            print('--CALCULATING dCSI--')
            postcsi = csi(postswir2, postnir)
            precsi = csi(preswir2, prenir)
            # combine so that nodata values (zero) in either index layer stay nodata in the output
            mask = postcsi*precsi!=0
            # Pre/post CSI difference
            dcsi = np.zeros(postcsi.shape).astype(rasterio.float32)
            dcsi[mask] = postcsi[mask] - precsi[mask]

            # Thresholding
            print('--CALCULATING THRESHOLDING--')
            thresholds = config.THRESHOLD 
            # print('Thresholds used: ', thresholds)
            burnseed = threshold_imgs(dsavi, postnbr, dnbr2, dcsi, thresholds)

            # Region growing
            print('--CALCULATING BURN REGIONS--')
            thresholds = config.GROW 
            # print('Thresholds used: ', thresholds)
            burnarray = grow_burn(dsavi, postnbr, dnbr2, dcsi, thresholds)
...
if __name__ == "__main__":
    ...
    toprocess, toprocess2 = getdatalist(wd, config.PROC_GRANULES, config.MONTHS_OUT, config.CLOUD, config.CLOUD2)
